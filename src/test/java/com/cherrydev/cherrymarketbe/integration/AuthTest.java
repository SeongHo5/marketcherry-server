...
final class AuthTest extends AbstractIntegrationTest {
  ...
  Stream<DynamicTest> dynamicTestForAuth() throws JsonProcessingException {
    ...
    return Stream.of(
        testHelper.testApiEndpoint(
            "사용자_인증-로그인_성공", signIn(requestSignInA), status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "사용자_인증-로그인_실패_비밀번호_오류", signIn(requestSignInB), status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "사용자_인증-로그인_실패_이메일_오류", signIn(requestSignInC), status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "사용자_인증-로그인_실패_계정_미활성화", signIn(requestSignInD), status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "사용자_인증-로그인_실패_탈퇴한_사용자", signIn(requestSignInE), status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "사용자_인증-로그인_실패_계정_존재하지_않음", signIn(requestSignInF), status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "사용자_인증-토큰_재발급_성공", reIssue(requestJwtA), status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "사용자_인증-토큰_재발급_실패_토큰_오류", reIssue(requestJwtB), status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "사용자_인증-로그아웃_성공", signOut(jwtToken.accessToken()), status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "사용자_인증-로그아웃_실패_토큰_오류", signOut(wrongAccessToken), status().is4xxClientError()));
  }
  ...
  Stream<DynamicTest> dynamicTestForEmailVerification() {
    ...
    return Stream.of(
        testHelper.testApiEndpoint(
            "사용자_인증-본인인증_메일_전송_성공", sendVerificationEmail(email), status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "사용자_인증-발송-실패_이미_발송됨", sendVerificationEmail(email), status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "사용자_인증-본인인증_성공", verifyEmail(email, verificationCodeA), status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "사용자_인증-본인인증_실패_이미_인증됨",
            verifyEmail(email, verificationCodeA),
            status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "사용자_인증-본인인증_확인_실패_잘못된_코드", verifyEmail(email, "111111"), status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "사용자_인증-비밀번호_재설정_메일_전송_성공", sendPasswordResetEmail(email), status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "사용자_인증-비밀번호_재설정_메일_전송_실패_이미_발송됨",
            sendPasswordResetEmail(email),
            status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "사용자_인증-비밀번호_재설정_성공",
            resetPassword(email, verificationCodeA),
            status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "사용자_인증-비밀번호_재설정_실패_잘못된_코드",
            resetPassword(email, "111111"),
            status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "사용자_인증-비밀번호_재설정_실패_잘못된_이메일",
            resetPassword(wrongEmail, verificationCodeA),
            status().is4xxClientError()));
  }

  private RequestBuilder sendVerificationEmail(String email) {
    return post("/api/auth/email-verification/send-mail").queryParam("email", email);
  }

  private RequestBuilder sendPasswordResetEmail(String email) {
    return post("/api/auth/password-reset/send-mail").queryParam("email", email);
  }
