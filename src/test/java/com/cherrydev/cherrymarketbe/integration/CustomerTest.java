...
final class CustomerTest extends AbstractIntegrationTest {
  ...
  Stream<DynamicTest> dynamicTestForCustomer() throws JsonProcessingException {
    ...
    return Stream.of(
        testHelper.testApiEndpoint(
            "고객-배송지_목록_조회", getMyAddress(accessToken), status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "고객-배송지_추가_성공",
            addAddress(requestAddAddressA, accessToken),
            status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "고객-배송지_추가_실패_최대_개수_초과",
            addAddress(requestAddAddressA, accessToken),
            status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "고객-배송지_추가_실패_기본_배송지_존재",
            addAddress(requestAddAddressB, accessToken),
            status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "고객-배송지_삭제_성공", deleteAddress(1L, accessToken), status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "고객-배송지_수정_성공",
            modifyAddress(requestModifyAddressA, accessToken),
            status().is2xxSuccessful()));
  }
  ...
  private RequestBuilder addAddress(RequestAddAddress request, String accessToken)
  ...
  private RequestBuilder modifyAddress(RequestModifyAddress request, String accessToken)
      throws JsonProcessingException {
    return post("/api/customer/address/")
        .header(AUTHORIZATION, BEARER_PREFIX + accessToken)
        .contentType(APPLICATION_JSON)
  ...
  private RequestBuilder deleteAddress(Long addressId, String accessToken)
      throws JsonProcessingException {
  ...
  Stream<DynamicTest> dynamicTestForCustomerReward() {
    ...
    return Stream.of(
        testHelper.testApiEndpoint(
            "고객-리워드_정보_조회", getMyRewardSummary(accessToken), status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "고객-쿠폰_등록_성공", registerCoupon(couponCode, accessToken), status().is2xxSuccessful()),
        testHelper.testApiEndpoint(
            "고객-쿠폰_등록_실패_이미_보유",
            registerCoupon(couponCode, accessToken),
            status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "고객-쿠폰_등록_실패_없는_코드",
            registerCoupon("NONE1515", accessToken),
            status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "고객-쿠폰_목록_조회", getCouponList(accessToken), status().is2xxSuccessful()));
  }
