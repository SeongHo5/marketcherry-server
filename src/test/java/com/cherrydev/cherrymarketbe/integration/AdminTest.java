...
final class AdminTest extends AbstractIntegrationTest {
  ...
  @WithUserDetails(
      value = "admin@devcherry.com",
      userDetailsServiceBeanName = "accountDetailsServiceImpl")
  Stream<DynamicTest> dynamicTestForAccountManagement() throws JsonProcessingException {
    ...
    return Stream.of(
        ...
        testHelper.testApiEndpoint(
            "관리자-계정_권한_변경_성공", modifyAccountRole(modifyUserRole), status().isOk()),
        testHelper.testApiEndpoint(
            "관리자-계정_상태_변경_성공", modifyAccountStatus(modifyUserStatusA), status().isOk()),
        testHelper.testApiEndpoint(
            "관리자-계정_상태_변경_실패_날짜_누락",
            modifyAccountStatus(modifyUserStatusB),
            status().is4xxClientError()),
        testHelper.testApiEndpoint(
            "관리자-계정_상태_변경_실패_날짜_오류",
            modifyAccountStatus(modifyUserStatusC),
            status().is4xxClientError()));
  }
  ...
  private RequestBuilder modifyAccountRole(ModifyUserRole modifyUserRole)
  ...
  private RequestBuilder modifyAccountStatus(ModifyUserStatus modifyUserStatus)
      throws JsonProcessingException {
  ...
  Stream<DynamicTest> dynamicTestForRewardAndCouponManagement() throws JsonProcessingException {
    ...
    return Stream.of(
        testHelper.testApiEndpoint(
            "관리자-포인트_지급_성공", grantReward(requestAddRewardA), status().isOk()),
        testHelper.testApiEndpoint(
            "관리자-포인트_지급_실패_없는_계정", grantReward(requestAddRewardB), status().is4xxClientError()),
        testHelper.testApiEndpoint("관리자-쿠폰_발급_성공", issueCoupon(issueCouponA), status().isOk()),
        testHelper.testApiEndpoint(
            "관리자-쿠폰_발급_실패_코드_중복", issueCoupon(issueCouponB), status().is4xxClientError()));
  }

  private RequestBuilder grantReward(RequestAddReward requestAddReward)
      throws JsonProcessingException {
