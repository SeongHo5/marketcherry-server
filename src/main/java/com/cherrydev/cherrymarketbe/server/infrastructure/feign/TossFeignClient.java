...
public interface TossFeignClient {
  ...
  TossPayment findPaymentByOrderId(@PathVariable("orderId") String orderId);

  @GetMapping("/{paymentKey}")
  TossPayment findPaymentByPaymentKey(@PathVariable("paymentKey") String paymentKey);
  ...
  TossPayment cancelPayment(
      @PathVariable("paymentId") String paymentId, @RequestBody PaymentCancelForm form);
}
