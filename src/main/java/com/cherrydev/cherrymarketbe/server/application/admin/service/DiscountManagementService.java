...
public class DiscountManagementService {
  ...
  public List<DiscountInfo> fetchAllDiscounts() {
    return discountRepository.findAll().stream().map(DiscountInfo::of).toList();
  }
  ...
  public DiscountInfo modifyDiscountDetail(
      final String discountCode, final RequestModifyDiscount request) {
  ...
  private Discount handleModifyInternal(
      final Discount discount, final RequestModifyDiscount request) {
