...
public class AddressService {
  ...
  @Transactional
  public void addAddress(final AccountDetails accountDetails, final RequestAddAddress request) {
  ...
  public List<AddressInfo> getAddress(final AccountDetails accountDetails) {
    Account account = accountDetails.getAccount();
    return addressRepository.findAllByAccount(account).stream().map(AddressInfo::of).toList();
  }

  @Transactional(readOnly = true)
  public CustomerAddress fetchDefaultAddressForOrders(final Account account) {
    return addressRepository
        .findByAccountAndIsDefault(account, true)
        .orElseThrow(() -> new NotFoundException(NOT_FOUND_ADDRESS));
  }

  @Transactional
  public void deleteAddress(final AccountDetails accountDetails, final Long addressId) {
    Account account = accountDetails.getAccount();
  ...
  public AddressInfo modifyAddress(
      final AccountDetails accountDetails, final RequestModifyAddress requestModifyAddress) {
    Account account = accountDetails.getAccount();
    ...
    CustomerAddress updatedAddress =
        handleModifyAddressInternal(customerAddress, requestModifyAddress);
  ...
  private CustomerAddress handleModifyAddressInternal(
      final CustomerAddress customerAddress, final RequestModifyAddress requestModifyAddress) {
