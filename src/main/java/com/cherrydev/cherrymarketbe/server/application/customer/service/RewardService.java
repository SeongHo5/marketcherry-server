...
public class RewardService {
  ...
  public RewardInfo getRewardSummaryAndList(final AccountDetails accountDetails) {
    ...
    List<RewardInfo.RewardItem> rewardItems =
        rewards.stream().map(this::generateRewardItem).toList();

    return RewardInfo.builder().summary(summary).rewards(rewardItems).build();
  }
  ...
  private RewardInfo.RewardSummary generateRewardSummary(final List<CustomerReward> rewards) {
    ...
    Integer expiredReward =
        rewards.stream()
            .filter(
                credit ->
                    !credit.getIsUsed() && credit.getExpiredAt().isBefore(LocalDate.now(clock)))
            .filter(reward -> !reward.getGrantType().isSameType(USE))
            .mapToInt(CustomerReward::getRewardAmount)
            .sum();
