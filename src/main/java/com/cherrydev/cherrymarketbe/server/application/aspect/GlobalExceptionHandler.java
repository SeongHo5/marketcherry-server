 ...
 *
 * <p>자세한 로그는 서버 로그로만 기록하고, 클라이언트에게는 상태 코드와 간단한 메세지만 전달하도록 구성한다.
 *
 * <p><i>요청, 예외에 대한 로깅은 AOP로 처리, 이곳에서는 예외 처리만 구성</i>
 */
...
public class GlobalExceptionHandler {
    ...
    return ResponseEntity.status(INTERNAL_SERVER_ERROR)
        .body(
            new ErrorResponse(
                ...
                ERROR_MESSAGE_500 + " : " + ex.getLocalizedMessage()));
  }
  ...
  protected ResponseEntity<ErrorResponse> handleInsufficientStockExceptionInternal(
  ...
  protected ResponseEntity<ErrorResponse> handleArgumentNotValidException(
    ...
    return ResponseEntity.status(BAD_REQUEST)
        .body(
            new ErrorResponse(
                BAD_REQUEST.value(), generateValidationErrorDetails(ex.getBindingResult())));
  }
  ...
  protected ResponseEntity<ErrorResponse> handleMethodNotSupportedException(
    ...
    String message =
        "지원하지 않는 메서드입니다. Expected: " + ex.getSupportedHttpMethods() + " Actual : " + ex.getMethod();
